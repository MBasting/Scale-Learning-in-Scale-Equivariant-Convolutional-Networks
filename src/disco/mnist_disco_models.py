...
class MNIST_DISCO(nn.Module):
    ...
    def __init__(self, pool_size=4, kernel_size=11, C1=32, C2=63, C3=95, linear_hidden = 256, scales=[1.0],
...
def mnist_disco(basis_min_scale, basis_save_dir, basis_mult, C1=32, C2=63, C3=95, linear_hidden = 256, **kwargs):
    q = 2 ** (1 / 3)
    scales = [q**i for i in range(4)]
    scales = [round(s, 2) for s in scales]
    model = MNIST_DISCO(pool_size=8, kernel_size=15, C1=C1, C2=C2, C3=C3, linear_hidden= linear_hidden, scales=scales, basis_type='disco_b',
