...
class DISCOBasisFree_ref(_Basis):
    def __init__(self, size, scales, effective_size, random=False):
        super().__init__(size=size, scales=scales, effective_size=effective_size)

        # Basis is only used for indexing, 
        # individual weights directly correspond to kernel values 
        approx = ApproximateProxyBasis(size, scales, effective_size)
        basis = approx.get_basis()
        basis = normalize_basis_by_min_scale(basis)
        self.register_buffer('basis', basis)
    
    def generate_perm(self, size):
        r=torch.randperm(size)
        c=torch.randperm(size)
        return r, c

    # def apply_perm(self, size):
