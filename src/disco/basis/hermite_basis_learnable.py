...
class HermiteBasisA_ref(_Basis):

    def __init__(self, kernel_size, scales, effective_size):
        super().__init__(size=kernel_size, scales=scales.cpu().detach().tolist(), effective_size=effective_size)
        basis = steerable_A(kernel_size, scales, effective_size)
        basis = normalize_basis_by_min_scale(basis)
        self.register_buffer('basis', basis)


class HermiteBasisB_ref(_Basis):

    def __init__(self, size, scales, effective_size, basis_mult, basis_max_order):
        super().__init__(size=size, scales=scales.cpu().detach().tolist(), effective_size=effective_size)
        basis = steerable_B(size, scales, effective_size,
...
def hermite_basis_varying_order(size, scale, max_order=None):
    ...
    X = torch.linspace(-(size // 2), size // 2, int(size), device=scale.device)
    Y = torch.linspace(-(size // 2), size // 2, int(size), device=scale.device)
...
def hermite_basis_varying_sigma(size, base_scale, max_order=4, mult=2, num_funcs=None):
    ...
    X = torch.linspace(-(size // 2), size // 2, int(size), device=base_scale.device)
    Y = torch.linspace(-(size // 2), size // 2, int(size), device=base_scale.device)
...
def steerable_A(size, scales, effective_size):
    ...
    basis_tensors = []
    for scale in scales:
        size_before_pad = int(size * (scale / max_scale).item()) // 2 * 2 + 1
        basis = hermite_basis_varying_order(size_before_pad, scale, max_order)
        basis = basis[None, :, :, :]
        pad_size = (size - size_before_pad) // 2
        if pad_size < 0:
            print("Padding below 0")
            print(size, size_before_pad, scale, max_scale)
...
def steerable_B(size, scales, effective_size, mult=1.4, max_order=4):
    ...
    basis_tensors = []
    for scale in scales:
        size_before_pad = int(size * (scale / max(scales)).item()) // 2 * 2 + 1
