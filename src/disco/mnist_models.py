...
from .ses_conv_learnable import SESConv_Z2_H, SESConv_H_H, SESMaxProjection


class MNIST_SESN(nn.Module):

    def __init__(self, pool_size=4, kernel_size=11, C1=32, C2=63, C3=95, scales=[1.0], linear_hidden= 256,
        ...
        self.linear = nn.Sequential(
            nn.Linear(4 * C3, linear_hidden, bias=False),
            nn.BatchNorm1d(linear_hidden),
            nn.ReLU(True),
            nn.Dropout(dropout),
            nn.Linear(linear_hidden, 10)
        )
...
def mnist_hermite(basis_min_scale=1.5, basis_mult=1.5, C1=32, C2=63, C3=95,  linear_hidden=256, **kwargs):
    q = 2 ** (1 / 3)
    scales = [basis_min_scale * q**i for i in range(4)]
    scales = [round(s, 2) for s in scales]
    model = MNIST_SESN(pool_size=8, kernel_size=15, C1=C1, C2=C2, C3=C3, scales=scales,
                       basis_type='hermite_b', linear_hidden = linear_hidden, basis_mult=basis_mult, basis_max_order=4, dropout=0.7)
    return model
