...
from disco.ses_conv_learnable import SESConv_H_H, SESConv_Z2_H, SESConv_H_H_1x1, SESMaxProjection
        ...
        if pool:
            self.conv1 = nn.Sequential(
                ...
                SESConv_Z2_H(in_planes, out_planes, kernel_size=7, effective_size=3,
                             stride=stride, padding=3, bias=False, scales=scales, basis_type='hermite_a')
            )
        else:
            if interscale:
                self.conv1 = SESConv_H_H(in_planes, out_planes, 2, kernel_size=7, effective_size=3, stride=stride,
                                         padding=3, bias=False, scales=scales, basis_type='hermite_a')
            else:
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, scales=[1.0],
                 pools=[False, False, False], interscale=[False, False, False], basis_type = 'hermite_a'):
...
def wrn_SESN(depth=16, nr_classes=10, widen_factor = 8, drop_rate=0.3, scales = [0.9,1.269,1.8], pools=[False, True, True], interscale = [False, False, False],**kwargs):
    return WideResNet(depth=depth, num_classes=nr_classes, widen_factor=widen_factor, dropRate=drop_rate, scales = scales, pools=pools, interscale=interscale)